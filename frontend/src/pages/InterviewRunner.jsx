...
function InterviewRunner() {
  ...
  // Local submitted state — locks question immediately without waiting for backend
  const [submittedLocal, setSubmittedLocal] = useState({});

  // Code + audio drafts saved in localStorage per session
  ...
  // Auto-detect language when session loads
  useEffect(() => {
  ...
  const isQuestionLocked   = isReduxSubmitted || isLocallySubmitted; // lock if either true
  const isProcessing       = isQuestionLocked && !currentQuestion?.isEvaluated; // still waiting for AI
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 pb-32">
      ...
      {/* ── Top bar: Role + question dots + finish button ── */}
      ...
      {/* ── AI Feedback panel — shown after question is evaluated ── */}
